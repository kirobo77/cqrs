...
${color_reset}
    \\curl -k -sSL https://zipkin.io/quickstart.sh | bash -sx -- $@
...
fetch() {
    url="$1"; shift
    target="$1"; shift
    execute_and_log curl -k -fL -o "'$target'" "'$url'"
}
...
fetch_latest_version() {
    ...
    printf $(curl -k -sSL $url | sed -n '/<version>/s/.*<version>\([^<]*\)<\/version>.*/\1/p'|tail -1)
}
